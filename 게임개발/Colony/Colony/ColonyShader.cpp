...
void BasicShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, D3D12_PRIMITIVE_TOPOLOGY_TYPE d3dPrimitiveTopology, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
...
DepthSkinnedRenderingShader::DepthSkinnedRenderingShader()
{
}
...
D3D12_INPUT_LAYOUT_DESC DepthSkinnedRenderingShader::CreateInputLayout()
{
	UINT nInputElementDescs = 7;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_SHADER_BYTECODE DepthSkinnedRenderingShader::CreateVertexShader()
{
	return(BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "VSSkinnedAnimationStandard", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE DepthSkinnedRenderingShader::CreatePixelShader()
{
	return (BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "PSDepthWriteShader", "ps_5_1", &m_pd3dPixelShaderBlob));
}

D3D12_BLEND_DESC DepthSkinnedRenderingShader::CreateBlendState()
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
...
D3D12_DEPTH_STENCIL_DESC DepthSkinnedRenderingShader::CreateDepthStencilState()
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = TRUE;
...
D3D12_RASTERIZER_DESC DepthSkinnedRenderingShader::CreateRasterizerState()
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
...
{
	if (nPipelineState == 0)
		return(BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "VSParticleStreamOutput", "vs_5_1", ppd3dShaderBlob));
	else
		return(BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "VSParticleDraw", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE ParticleShader::CreateGeometryShader(ID3DBlob** ppd3dShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
		return(BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "GSParticleStreamOutput", "gs_5_1", ppd3dShaderBlob));
	else
		return(BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "GSParticleDraw", "gs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE ParticleShader::CreatePixelShader(ID3DBlob** ppd3dShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
		return(BasicShader::CreatePixelShader());
	else
		return(BasicShader::CompileShaderFromFile(L"Shaders.hlsl", "PSParticleDraw", "ps_5_1", ppd3dShaderBlob));
}
...
D3D12_INPUT_LAYOUT_DESC ParticleShader::CreateInputLayout(int nPipelineState)
{
	...
	pd3dInputElementDescs[2] = { "DIRECTION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 16, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[3] = { "ACC", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 28, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[4] = { "SPEED", 0, DXGI_FORMAT_R32_FLOAT, 0, 40, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[5] = { "LIFETIME", 0, DXGI_FORMAT_R32_FLOAT, 0, 44, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[6] = { "AGE", 0, DXGI_FORMAT_R32_FLOAT, 0, 48, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
	pd3dInputElementDescs[7] = { "STARTTIME", 0, DXGI_FORMAT_R32_FLOAT, 0, 52, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 };
...
D3D12_STREAM_OUTPUT_DESC ParticleShader::CreateStreamOuputState(int nPipelineState)
{
	...
	if (nPipelineState == 0)
	{
		...
		pd3dStreamOutputDecls[1] = { 0, "TYPE", 0, 0, 1, 0 };
		pd3dStreamOutputDecls[2] = { 0, "DIRECTION", 0, 0, 3, 0 };
		pd3dStreamOutputDecls[3] = { 0, "ACC", 0, 0, 3, 0 };
		pd3dStreamOutputDecls[4] = { 0, "SPEED", 0, 0, 1, 0 };
		pd3dStreamOutputDecls[5] = { 0, "LIFETIME", 0, 0, 1, 0 };
		pd3dStreamOutputDecls[6] = { 0, "AGE", 0, 0, 1, 0 };
		pd3dStreamOutputDecls[7] = { 0, "STARTTIME", 0, 0, 1, 0 };
...
void ParticleShader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, int nPipelineState)
{
	...
	d3dPipelineStateDesc.NumRenderTargets = (nPipelineState == 0) ? 0 : 1;
	d3dPipelineStateDesc.RTVFormats[0] = (nPipelineState == 0) ? DXGI_FORMAT_UNKNOWN : DXGI_FORMAT_R8G8B8A8_UNORM;
...
void ParticleShader::RenderShader(ID3D12GraphicsCommandList* pd3dCommandList, Camera* pCamera, int nPipelineState)
{
