...
DogMonster::DogMonster(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ResourceManager* pResourceManager, float scale)
{
	...
	CLoadedModelInfo* pSpider = pResourceManager->BringModelInfo("Model/Chomper.bin", "Model/Textures/Chomper/");
	...
	//m_pBrain
	m_pBrain = new DogGoalThink(this);
	//Soul
	m_pSoul = new DogAIController(m_pBrain, this);
	m_pSoul->m_pAnimationControl = ((DogAnimationController*)m_pSkinnedAnimationController);
...
void DogMonster::Update(float fTimeElapsed)
{
	//판단	
	m_pBrain->Process();
	//실행
...
void DogMonster::UpdatePosition(float fTimeElapsed)
{
...
void DogAnimationController::AdvanceTime(float fElapsedTime, GameObject* pRootGameObject)
{
...
void DogAnimationController::AdvanceTimeWithMultithread(float fElapsedTime, GameObject* pRootGameObject, int idx)
{
